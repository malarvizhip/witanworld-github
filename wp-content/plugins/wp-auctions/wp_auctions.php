...
*/
  ...
  v3.0 Free  - OwenC - 14/10/14 - Refreshed with premium features
  ...
  v3.6.3 - OwenC - 08/03/2016 - Bug Fixes
  v3.6.5 - HyderJ/OwenC - 14/4/2016 - v4.5 compatibility
  v3.6.5 - OwenC - 1/3/2016 - Switch to wp_mail functionality
*/
  ...
  $bid_type = "user";
    ...
    if ($result == '') {
       ...
       // Let's also check that the bid is in the right range for the (piggyback staticpage)
  		 $table_name = $wpdb->prefix . "wpa_auctions";
			 $strSQL = "SELECT current_price,start_price,staticpage,BIN_price FROM $table_name WHERE id=".$auction_id;
       ...
       if ($rows->start_price < $max_bid) $result=__("Sorry, your bid is too high",'wpauctions');
       elseif ($rows->current_price != NULL && ($rows->current_price <= $max_bid)) $result=__("Sorry, your bid must be lesser than the current bid price",'wpauctions');
       elseif ($rows->current_price != NULL && ($rows->current_price - wpa_get_increment($rows->current_price) < $max_bid)) $result=__("Sorry, your bid must be lesser than",'wpauctions')." ".$currencysymbol.number_format($rows->current_price - wpa_get_increment($rows->current_price), 2, '.', ',');;

       // override bidding process if auction in a "Buy It Now"
       if($rows->BIN_price >= ($max_bid + 0)){
          $BIN_amount = ($max_bid + 0);
       }
       ...
       if ($result=='') {
		   ...
		   $table_name = $wpdb->prefix . "wpa_bids";
		   $strSQL = "SELECT * FROM $table_name WHERE auction_id=".$auction_id." ORDER BY current_bid_price ASC LIMIT 1";
...
function check_auction_end($auction_id) {
   ...
   if ($rows[0] == $auction_id && $rows[1] == 0 && $rows[2] == '') {
      ...
      // prepare result
      $table_name = $wpdb->prefix . "wpa_bids";
	    $strSQL = "SELECT bidder_name, bidder_email, date, current_bid_price FROM $table_name WHERE auction_id=".$auction_id." ORDER BY current_bid_price ASC, bid_type LIMIT 1";
...
$printstring .= '<h6>'.__('Place Your Bid Here','wpauctions').'</h6><span> [<a href="http://www.wpauctions.com/faq/" target="_blank" rel="nofollow">?</a>]</span>';
...
<div class="wrap wp-auctions">
	...
	<div class="wpa-update-screen">
		...
		<p><a href="http://www.wpauctions.com/download" class="button button-primary">Only <del>$49</del> <strong style="text-decoration: underline;">$45</strong>, click for Instant Download</a></p>
	</div>
...
function wpa_chkPrices($StartPrice, $ReservePrice,$BINPrice)
{
  ...
  elseif($ReservePrice > 0 && ($StartPrice - $ReservePrice) < 0):
		$bitError = 1;
	//elseif($BINPrice > 0 && ($BINPrice - $StartPrice) < 0):
  elseif($BINPrice > 0 && ($StartPrice - $BINPrice) < 0):
		$bitError = 1;
	endif;
   ...
   if($_POST["wpa_action"] == "Add Auction"):
      ...
      elseif(wpa_chkPrices($strSaveStartPrice,$strSaveReservePrice,$strSaveBINPrice) == 1):
         //$strMessage = __('Starting Price must be numeric and less than Reserve and BIN Prices','wpauctions');
          $strMessage = __('Starting Price must be numeric and greater than BIN Prices','wpauctions');
      endif;
   ...
   <link href="../wp-content/plugins/wp-auctions/requisites/style.css" rel="stylesheet" type="text/css" />
	...
	<div class="wrap wp-auctions">
		...
		<div class="wpa-update-screen">
			...
			<p><a href="http://www.wpauctions.com/download" class="button button-primary">Only <del >$49</del> <strong style="text-decoration: underline;">$45</strong>, click to purchase</a></p>
		</div>
...
?>        
      ...
      <tr valign="top"> 
        ...
        <th scope="row"><?php _e('Max Price:','wpauctions') ?></th> 
        ...
        <?php _e('Specify the maximum price for your auction. Leave empty (or 0) for Fixed Price BIN','wpauctions') ?>
...
?>
	...
	<div class="wpa-update-screen">
		...
		<p><a href="http://www.wpauctions.com/download" class="button button-primary">Go Pro today <del>$49</del> <strong style="text-decoration: underline;">$45</strong>, save $10!</a></p>
	</div>
